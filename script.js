document.addEventListener('DOMContentLoaded', function() {
    ...
    const SUPABASE_URL = 'https://uurvzfyfehroaasdboyo.supabase.co';
    const SUPABASE_ANON_KEY = 'sb_publishable_4akKV3hzQ4susSjXR9dOgQ_sYc0q-nx';
    ...
    function updateProfileDisplay() {
        ...
        if (nameSpan) nameSpan.innerText = userProfile.name || currentUser?.user_metadata?.name || '—';
    ...
    // ---------- Работа с тренировками через Supabase ----------
    ...
    async function addWorkout(workoutData) {
        ...
        const { error } = await supabase
            .from('workouts')
            .insert([{ user_id: currentUser.id, ...workoutData }]);
        if (error) {
            alert(error.message);
            return false;
        }
    ...
    function initProfilePage() {
        ...
        currentUser = session.user;
        loadUserProfile();
        document.getElementById('editName').value = userProfile.name || currentUser.user_metadata?.name || '';
    ...
    function initLoginPage() {
        ...
        if (sendCodeBtn) {
            sendCodeBtn.addEventListener('click', async () => {
                ...
                else alert(translations[currentLang].recoverSent);
                modal.style.display = 'none';
            });
        }
    }
    ...
    // Тёмная тема и язык (глобальные настройки)
