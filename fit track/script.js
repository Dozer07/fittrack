...
// Дополнительные данные пользователя (рост, вес, настройки, категории)
let userProfile = {
    name: '',
    height: '',
    weight: '',
...
const translations = {
    ru: {
        ...
        notificationNoWorkouts: "Сегодня тренировок нет. Хорошего дня!",
    ...
    en: {
        ...
        notificationNoWorkouts: "No workouts today. Have a great day!",
...
function setToken(token) { localStorage.setItem('fittrack_token', token); }
function removeToken() { localStorage.removeItem('fittrack_token'); }

// ---------- Профиль пользователя (рост, вес, уведомления, категории) ----------
...
function loadCategories() {
    const saved = localStorage.getItem('fittrack_categories');
    if (saved) {
        userProfile.categories = JSON.parse(saved);
    } else {
...
function saveCategories() {
    localStorage.setItem('fittrack_categories', JSON.stringify(userProfile.categories));
}
...
async function addWorkout(workoutData) {
    if (!validatePositiveNumber(workoutData.duration, 'Длительность')) return false;
    if (!validatePositiveNumber(workoutData.calories, 'Калории')) return false;
    const response = await apiCall('/workouts', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    if (isWorkoutsPage) {
        tbody.innerHTML = currentUser.workouts.map(w => `
            <tr>
                ...
                <td>${w.date}</td>
                <td>${w.duration} ${t.workoutDuration.split(' ')[0]}</td>
                <td>${w.calories} ${t.workoutCalories.split(' ')[0]}</td>
...
function renderCalendar() {
    ...
    if (!currentUser || !currentUser.workouts) {
...
async function generateReport() {
    ...
    // Построить график по дням внутри периода
...
async function initMainPage() {
    ...
    try {
        ...
        if (response.ok) {
            ...
            currentUser = { id: tokenPayload.id, email: tokenPayload.email, name: tokenPayload.name, workouts: workouts };
            updateUIAfterLogin();
        } else throw new Error('Unauthorized');
    } catch (e) { removeToken(); window.location.href = 'login.html'; return; }
    const logoutBtn = document.getElementById('logoutBtn');
...
async function initWorkoutsPage() {
    ...
    if (addPresetBtn) {
        addPresetBtn.addEventListener('click', async () => {
            ...
            if (preset === 'Силовая' || preset === 'Strength') { duration = 45; calories = 250; }
            else if (preset === 'Кардио' || preset === 'Cardio') { duration = 30; calories = 280; }
            else if (preset === 'Интервальная' || preset === 'Interval') { duration = 25; calories = 220; }
    ...
    if (saveCustomWorkout) {
        saveCustomWorkout.addEventListener('click', async () => {
            ...
            if (!validatePositiveNumber(duration, 'Длительность')) return;
            if (!validatePositiveNumber(calories, 'Калории')) return;
...
function initProfilePage() {
    ...
    if (!token) { window.location.href = 'login.html'; return; }
    const tokenPayload = JSON.parse(atob(token.split('.')[1]));
    currentUser = { id: tokenPayload.id, email: tokenPayload.email, name: tokenPayload.name };
    loadUserProfile();
    ...
    if (saveBtn) {
        saveBtn.addEventListener('click', () => {
            ...
            if (height && !validatePositiveNumber(height, 'Рост')) return;
            if (weight && !validatePositiveNumber(weight, 'Вес')) return;
...
function initSettingsPage() {
    ...
    if (deleteAccountSettingsBtn) {
        deleteAccountSettingsBtn.addEventListener('click', async () => {
            if (confirm(translations[currentLang].accountDeleteConfirm)) alert('Удаление аккаунта не реализовано в демо-версии.');
        });
    }
    const logoutBtn = document.getElementById('logoutBtn');
    if (logoutBtn) logoutBtn.addEventListener('click', logout);
...
async function initReportPage() {
    ...
    // Установим даты по умолчанию: последние 30 дней
    const today = new Date();
...
function initLoginPage() {
    const loginBtn = document.getElementById('loginBtn');
    if (loginBtn) loginBtn.addEventListener('click', async () => { await login(document.getElementById('loginEmail').value, document.getElementById('loginPassword').value); });
    const modal = document.getElementById('recoverModal');
    const forgotLink = document.getElementById('forgotPasswordLink');
    if (forgotLink) forgotLink.addEventListener('click', (e) => { e.preventDefault(); if (modal) modal.style.display = 'flex'; });
    const closeModal = document.getElementById('closeModal');
    if (closeModal) closeModal.addEventListener('click', () => { if (modal) modal.style.display = 'none'; });
    const sendCodeBtn = document.getElementById('sendRecoverCode');
    if (sendCodeBtn) sendCodeBtn.addEventListener('click', async () => { alert(translations[currentLang].recoverSent); if (modal) modal.style.display = 'none'; });
}

function initRegisterPage() {
    const registerBtn = document.getElementById('registerBtn');
    if (registerBtn) registerBtn.addEventListener('click', async () => { await register(document.getElementById('regEmail').value, document.getElementById('regPassword').value, document.getElementById('regName').value); });
}
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    else if (path.includes('workouts.html')) await initWorkoutsPage();
    else if (path.includes('report.html')) initReportPage();
    else await initMainPage();
});